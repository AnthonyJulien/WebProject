        ...
        <tbody>
            ...
            @foreach ($getCartItems as $item) {{-- $getCartItems is passed in from cart() method in Front/ProductsController.php --}}
                ...
                <tr>
                        ...
                        <div class="cart-price">
                            ...
                            @if ($getDiscountAttributePrice['discount'] > 0) {{-- If there's a discount on the price, show the price before (the original price) and after (the new price) the discount --}}
                                <div class="price-template">
                                    ...
                                    {{ Session::get('currency') }} {{ ConvertPrice(Session::get('currency'), $getDiscountAttributePrice['final_price']) }}
                                    </div>
                                    <div class="item-old-price" style="margin-left: -40px">
                                    {{ Session::get('currency') }} {{ ConvertPrice(Session::get('currency'), $getDiscountAttributePrice['product_price']) }}
                                    </div>
                                </div>
                    ...
                    <td>
                        <div class="cart-price">
                        {{ Session::get('currency') }} {{ ConvertPrice(Session::get('currency'), $getDiscountAttributePrice['final_price']) * $item['quantity'] }} {{-- price of all products (after discount (if any)) (= price (after discoutn) * no. of products) --}}
                        </div>
                    </td>
                    <td>
                ...
                @php $total_price = $total_price + (ConvertPrice(Session::get('currency'), $getDiscountAttributePrice['final_price']) * $item['quantity']) @endphp
            @endforeach
        ...
        <table>
            ...
            <tbody>
                <tr>
                    ...
                    <td>
                        <span class="calc-text">{{ Session::get('currency') }} {{ $total_price }}</span>
                    </td>
                </tr>
                <tr>
                    <td>
                    ...
                    <td>
                        <span class="calc-text couponAmount"> {{-- We create the 'couponAmount' CSS class to use it as a handle for AJAX inside    $('#applyCoupon').submit();    function in front/js/custom.js --}}
                            ...
                            {{ Session::get('currency') }} {{ \Illuminate\Support\Facades\Session::get('couponAmount') }}
                            @else
                            {{ Session::get('currency') }} 0
                            @endif
                        </span>
                    </td>
                </tr>
