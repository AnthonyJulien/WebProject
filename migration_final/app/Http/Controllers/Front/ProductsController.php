...
public function checkout(Request $request) {
    ...
    // Get the Cart Items of a certain user (using their `user_id` if they're authenticated/logged in or their `session_id` if they're not authenticated/not logged in (guest))    
    $getCartItems = Cart::getCartItems();
    ...
    if ($request->isMethod('post')) { // if the <form> in front/products/checkout.blade.php is submitted (the HTML Form that the user submits to submit their Delivery Address and Payment Method)
        ...
        // Website Security
        // Preventing orders with 'disabled' products or attributes and preventing orders with out-of-stock products.
        foreach ($getCartItems as $item) {
        ...
        // Validation for address, payment gateway, and T&C agreement
        ...
        // If the vendor's ID is greater than 0, implying the product is sold by a vendor
if ($getProductDetails['vendor_id'] > 0) {
    // Retrieve vendor details to get the email
    $vendorDetails = Vendor::where('id', $getProductDetails['vendor_id'])->first();

    if ($vendorDetails) {
        // Sending email to the vendor
        ...
        \Illuminate\Support\Facades\Mail::send('emails.vendor_order', $vendorMessageData, function ($message) use ($vendorEmail) {
            $message->to($vendorEmail)->subject('New Order Notification - MultiVendorEcommerceApplication.com.eg');
        });
    }
}
    ...
    public function UpdateCurrency(Request $request) {
        if ($request->ajax()) { // if the request is coming via an AJAX call
            ...
            //updating the session currency with the selected value from the list 
